...
var rdata = (mydate.getFullYear() + (mydate.getMonth() + 1) + mydate.getDate()).toString();

function numRandom(min, max){
    return min + Math.floor(Math.random() * (max - min + 1));
}
var chushiimg=numRandom(1,9);
console.log(chushiimg);
function puzzleGame(contentArea, level) {
    $(".orgimage img").attr("src",'images/random/'+chushiimg+'.jpg');
    //显示图片节点
    this.$contentArea = $(contentArea)
    this.imageSrc = 'images/random/'+chushiimg+'.jpg';
    ...
    // 初始游戏本地存储
    // this.browserStorage(this.count);
    //绑定全局事件
    this.creatEvent();
    // 实时获取
    this.objs=JSON.parse(BrowserStorage.api.get(rdata).value);
}
puzzleGame.prototype = {
    ...
    startGame: function(successCallback) {
        ...
        if(duixiang==null){
            ...
            this.tttttt=this.ctrolTimes(this);
        }else{
            //2016-12-23 修改 一会儿还原
...
// ---------------------------------------------------------------------------------------------------------------------------------------------
    ctrolTimes:function(aaaaa){
        var sdsdsds=setInterval(times,1000);
        function times(){
            aaaaa.time=parseInt(aaaaa.time)-1;
            $(".time span").html(aaaaa.creatTimeText(aaaaa.time));
            var _cc_count;
            var duixiang=BrowserStorage.api.get(rdata).value;
            if(duixiang==null){
                _cc_count=0;
            }else{
                _cc_count=JSON.parse(BrowserStorage.api.get(rdata).value).count;
            }
            console.log(_cc_count);
            if(aaaaa.time<=0){
                if(aaaaa.state== "successful"){
                    $(".game_end").fadeIn();
                    $("._time_end").fadeIn();
                    $("._end").fadeOut();
                    $(".game_frequency").html(3-_cc_count+'次');
                    clearInterval(sdsdsds);
                }else if(aaaaa.state=="failure"){
                    $(".game_end").fadeIn();
                    $("._time_end").fadeIn();
                    $("._end").fadeOut();
                    $(".game_frequency").html(3-_cc_count+'次');
...
// --------------------------------------------------------------------------------------------------------------------------------------------
    ...
    touchmove: function(event) {
        ...
        var deltaX = event.touches[0].pageX - this.start.pageX;
        var deltaY = event.touches[0].pageY - this.start.pageY;
        yidongX = event.touches[0].pageX;
        yidongY = event.touches[0].pageY
            //元素移动的距离
        this.element.css({
    ...
    calculateOverflow: function(event) {
        var offsetX = parseInt($(this.preTarget).css("left"));
        var offsetY = parseInt($(this.preTarget).css("top"));
        ...
        var offsetRight = pageX + this.debrisWidth;
        var offsetTop = pageY + offsetY;
        var offsetBottom = pageY;
        //左边边界
        if (offsetLeft < (range.left - overflowWidth)) {
            return true;
        }
        //右边边界
        if (offsetRight > (range.right + this.debrisWidth)) {
            return true;
        }
        //顶部边界
    ...
    calculateOverlap: function(event, startDebrisIndex) {
        ...
        if (event.type == "touchend") {
            var col = Math.floor((yidongY - this.contentTop) / this.debrisWidth),
                row = Math.floor((yidongX - this.contentLeft) / this.debrisHeight);
        } else if (event.type == "touchstart") {
            var col = Math.floor((event.touches[0].pageY - this.contentTop) / this.debrisWidth),
                row = Math.floor((event.touches[0].pageX - this.contentLeft) / this.debrisHeight);
        }
        //从整数1开始算起
        col = col + 1; //列
        row = row + 1; //行
    ...
    creatEvent: function() {
        ...
        var self = this;
        this.$contentArea[0].addEventListener("touchstart", function(event) {
            stopBehavior(event)
            self.touchstart(event)
        }, false)
        this.$contentArea[0].addEventListener("touchmove", function(event) {
            stopBehavior(event)
            self.touchmove(event)
        }, false)
        this.$contentArea[0].addEventListener("touchend", function(event) {
            stopBehavior(event)
            self.touchend(event)
        }, false)
    },
